...
public class PlayerAnimation : MonoBehaviour
{
    ...
    private void Awake()
    {
        gameManager.onGameStart += () => Play("idle"); // Subscribe to the game start event to play the idle animation
        fishingMode.onFishingEnd += () => Play("idle");
    }

    private void OnDestroy()
    {
        gameManager.onGameStart -= () => Play("idle"); // Unsubscribe from the event to avoid memory leaks
