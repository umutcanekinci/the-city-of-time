...
public class TaskManager : MonoBehaviour
{
    ...
    [SerializeField] private GameManager gameManager; // Oyun yöneticisi referansı

    public void Awake()
    {
        dialogue.onDialogueEnd += OnDialogueEnd;
        gameManager.onGameStart += ResetTasks;
    }
    ...
    public void ResetTasks() // Tüm görevleri sıfırla
    {
        foreach (Task task in tasks) // Görevleri döngü ile gez
        {
            task.ResetTask(); // Her görevi sıfırla
        }
        SetTask(0); // İlk göreve geri dön
    }
